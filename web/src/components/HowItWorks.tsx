...
import { Globe, Link2, ShieldCheck, ArrowRight, Copy } from "lucide-react";
    ...
    <section aria-labelledby="how-it-works">
        ...
        >
          {steps.map((s, i) => (
            <motion.div key={i} variants={fadeInUp} className="relative">
              <Card className="group relative rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm transition-all hover:border-white/20 hover:shadow-[0_10px_40px_-10px_rgba(0,0,0,.5)]">
                ...
                {s.snippet && (
                  <>
                    <div className="mt-4">
                      ...
                      >
                        {open === i ? "Hide sample" : "Show sample"}
                      </button>
                    </div>
              ...
              {i < steps.length - 1 && (
                <ArrowRight className="hidden md:block absolute -right-4 top-1/2 h-8 w-8 -translate-y-1/2 text-white/30" />
              )}
            </motion.div>
          ))}
        </motion.div>
      ...
      <div className="mt-8 grid gap-3 text-xs text-white/60 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-white/10 bg-black/30 p-3">
          ...
          <div>Portal & embeddable widget with real-time status (Active/Revoked/Expired).</div>
        </div>
      </div>
    </section>
  );
